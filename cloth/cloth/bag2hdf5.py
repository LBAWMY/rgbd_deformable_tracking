...
def flatten_msg(msg, t, max_strlen=None):
    ...
    for i, attr in enumerate(msg.__slots__):
        ...
        elif rostype == 'geometry_msgs/TransformStamped[]':
            ps = getattr(msg, attr)
            # print('ps: ', ps)
            for p in ps:
                ...
                result.extend([translation.x, translation.y, translation.z])
                result.extend([rotation.x, rotation.y, rotation.z, rotation.w])

        elif rostype == 'sensor_msgs/Image':
            p = getattr(msg, attr)
            for i in range(len(p.data)):
                result.extend([p.data[i]])
    ...
    for i, attr in enumerate(msg.__slots__):
        ...
        if rostype == 'std_msgs/Header':
            ...
            for i in range(len(ps)):
                ...
                # rotation = transform.rotation
                result.extend([(attr + '_translation' + '_x' + str(i), np.float32),
                               (attr + '_translation' + '_y' + str(i), np.float32),
                               (attr + '_translation' + '_z' + str(i), np.float32)])
                result.extend([(attr + '_rotation' + '_x' + str(i), np.float32),
                               (attr + '_rotation' + '_y' + str(i), np.float32),
                               (attr + '_rotation' + '_z' + str(i), np.float32),
                               (attr + '_rotation' + '_w' + str(i), np.float32)])
